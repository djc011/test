program mp2
implicit none
integer::i,j,ierr,k,l,ij,kl,ijkl,indx,ik,jl,ikjl,c,p,q,r,s,pqrs,ia,ja,jb,ib,iajb,ibja,a,b,pq,rs
...
double precision,dimension(7,7)::f,d1
double precision,dimension(665)::tei_ao,tei_mo
double precision:: nuc,val,e0,e1,deltae,emp2,start,finish
    ...
    do j=1,7
      ...
      do k=1,7
        do l=1,7
          ...
          jl = INDX(j,l)
          ikjl = INDX(ik,jl)
          f(i,j) = f(i,j) + d0(k,l) * (2.0 * TEI_ao(ijkl) - TEI_ao(ikjl))
        enddo
      enddo
    enddo
  enddo
            ...
            do q=1,7
              ...
              do r=1,7
                do s=1,7
                  rs = INDX(r,s)
                  pqrs = INDX(pq,rs)
                  TEI_MO(ijkl) =tei_mo(ijkl) + C0(p,i) * C0(q,j) * C0(r,k) * C0(s,l) * TEI_AO(pqrs)
                enddo
              enddo
            enddo
           enddo
    ...
    do a=6,7
      ...
      do j=1,5
        ...
        do b=6,7
          ...
          iajb = INDX(ia,jb)
          ibja = INDX(ib,ja)
          Emp2 = emp2 + TEI_mo(iajb) * (2 * TEI_mo(iajb) - TEI_mo(ibja))/(epsilon(i,i) + epsilon(j,j) - epsilon(a,a) - epsilon(b,b))
        enddo
      enddo
    enddo
  enddo
